...
def start(message):
    if message.chat.type in ['group', 'supergroup']:
        sending_mess = "<b>Привет!</b> Я бот для поиска видео на YouTube. Используйте команду /youtube [запрос] для поиска видео.\nДля просмотра всех доступных команд введите /help"
    else:
        sending_mess = "<b>Привет!</b> Это Ваш бот для поиска видео на YouTube, напишите что хотели бы найти...\nДля просмотра всех доступных команд введите /help"
...
/youtube [запрос] - Поиск видео на YouTube по вашему запросу
...
def youtube_command(message):
    ...
    query = message.text[len('/youtube '):].strip()
    if not query:
        bot.send_message(message.chat.id, "Пожалуйста, укажите поисковый запрос после команды /youtube")
        return
        
    busy_message = bot.send_message(message.chat.id, "Поиск на YouTube...")
